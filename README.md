...
- **Ligne 43**: `bool ConfigValidator::validateBraces(const std::vector<std::string>& lines){ //todo oo correct this func`
- **Ligne 78**: `bool ConfigValidator::validateSyntaxe(const std::vector<std::string>& lines){//todo correct this func`
- **Ligne 115**: `void ConfigValidator::setError(const std::string& error){} // TODO`
- **Ligne 248**: `if (codeNum < 400 || codeNum > 599){ // Todo check error code min max, and if we're gonne use them all`
- **Ligne 262**: `for (std::map<std::string, std::string>::const_iterator it = config.locations.begin(); //todo add locationS to config.`
...
- **Ligne 43**: `bool ConfigValidator::validateBraces(const std::vector<std::string>& lines){ //todo oo correct this func`
- **Ligne 255**: `if (codeNum < 400 || codeNum > 599){ // Todo check error code min max, and if we're gonne use them all`
- **Ligne 311**: `setError("should we accept empty method"+ methods ); //todo maybe accept some empty methods`
- **Ligne 318**: `if (m != "GET" && m != "POST" && m != "DELETE" ){ // todo can add more if bonuses`
- **Ligne 328**: `setError("Invalid client max body size format: " +size); //todo ask bebou for MBS`
- **Ligne 333**: `if (sizeNum <= 0 || sizeNum > 1000000000){ //todo decrease bc 1 gb is kinda overkill see commetn above`
- **Ligne 356**: `bool ConfigValidator::validateTimeout(const std::string& timeout){ //todo`
- **Ligne 362**: `bool ConfigValidator::validateRedirection(const std::string& redir){ //todo`
- **Ligne 367**: `bool ConfigValidator::validateIndex(const std::string& index){ //todo`
- **Ligne 373**: `bool ConfigValidator::validatePath(const std::string& path){ //todo`
- **Ligne 378**: `bool ConfigValidator::isPortUsed(int port) const{ //todo do we need a check for free port?`
