...
/*   Updated: 2025/09/08 14:05:22 by jim              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int ac, char **av){
	...
	if (!parser.validateBraces(configFile)){
	...
	try{
		servers = parser.parseAllServer(configFile);
		locations = parser.parseLocation(configFile);
	...
	for (size_t i = 0 ; i < servers.size(); i++){
		std::cout << "\n ===== Server : "<< parser.getServerDirective(servers[i], "server_name") << std::endl;
		std::cout << "\n ===== Host : "<< parser.getServerDirective(servers[i], "host") << std::endl;
		std::cout << "\n ===== root : "<< parser.getServerDirective(servers[i], "root") << std::endl;
		std::cout << "\n ===== index : "<< parser.getServerDirective(servers[i], "index") << std::endl;
